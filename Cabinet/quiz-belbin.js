...
(function () {
  ...
  function renderTotal() {
    ...
    const remaining = BELBIN_POINTS_PER_QUESTION - total;
    totalEl.textContent =
      remaining === 0
        ? "Распределены все 10 баллов ✓"
        : "Осталось распределить: " + remaining + " из " + BELBIN_POINTS_PER_QUESTION + " баллов";
  ...
  function renderQuestion() {
    ...
    q.options.forEach((opt, i) => {
      ...
      input.addEventListener("input", function () {
        ...
        // Нельзя распределить больше 10 баллов на вопрос суммарно — если
        // введённое число превышает то, что ещё осталось (10 минус то, что
        // уже стоит в остальных полях этого вопроса), обрезаем его до
        // максимально доступного остатка и сразу показываем это в поле,
        // чтобы не было путаницы, почему "Далее" не активна.
        const otherSum = questionTotal(current) - answers[current][i];
        const maxAllowed = BELBIN_POINTS_PER_QUESTION - otherSum;
